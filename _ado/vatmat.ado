...
program define vatmat, rclass
	...
	* Define lists of sectors : exempted, mixed, and non-mixed sectors 
	...
	dis "Program ended "
	dis "list of sectors to not be expanded : `collsecs'"
...
*Renaming sectors 
merge m:1 `sector' using `odata', assert(master matched) keepusing(`exempt' `pexempt') nogen
...
foreach var of varlist sector_*{
	...
	*replace `var' = `var'*`factor'
	if exempted==1 in `i' local exm = "Exempt"
	if exempted==0 in `i' local exm = "Not exempt"
	local sect = `sector'[`i']
